...
# region Client OAuth2 flows: real Keycloak integration


def _keycloak_token_url(keycloak_url: str) -> str:
    return f"{keycloak_url}/realms/oapi-test/protocol/openid-connect/token"


def _keycloak_oidc_discovery_url(keycloak_url: str) -> str:
...
def test_oauth2_password_grant_against_a_real_keycloak(
    ...
    client: Client = Client(
        ...
        oauth2_token_url=_keycloak_token_url(keycloak_url),
        timeout=10,
    )
    with client._request_oauth2_password_authorization() as response:
        token_response: dict[str, object] = json_module.loads(response.read())
...
def test_oidc_discovery_derives_the_real_token_url(
    ...
    client: Client = Client(
        open_id_connect_url=_keycloak_oidc_discovery_url(keycloak_url),
        timeout=10,
    )
    assert client._get_oauth2_token_url() == _keycloak_token_url(keycloak_url)
...
def test_client_credentials_grant_authenticates_a_real_protected_request(
    ...
    with client.request(
        ...
        userinfo: dict[str, object] = json_module.loads(response.read())
